...
| one key dropped from the payload `writeCheckpoint` is given | eight tests, each naming the key: `recovered 319 keys, want 320`, and an acknowledged key absent at four of the ten crash points in the checkpoint path |
...
| the same argument | the checkpoint is a fold of the *complete* prefix | **measured.** Drop one key from the payload `writeCheckpoint` is handed and eight tests fail on data, each naming the key that went - `recovered 319 keys, want 320`, and an acknowledged key absent at four of the ten crash points in the checkpoint path. The tests existed already; the measurement did not |
